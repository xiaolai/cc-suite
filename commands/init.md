...
Parse the JSON output. Extract `default_model` and use it as the suggested default model in the generated config. For compatibility with older preflight output, fall back to the first entry in `models`.
...
- **Default model**: {default_model from preflight, e.g. "gpt-5.6-sol"}
...
Then mirror the new server entry into `.codex/config.toml` and `.agents/mcp_config.json` so Codex and Antigravity can see the project MCP surface:
