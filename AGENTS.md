...
The SessionStart hook (`scripts/session-lifecycle-hook.mjs`) self-heals the two things that rot on their own in a project nobody has run a command in: the dead `codex-cli` MCP entry, and a `.claude/skills/cc-suite` symlink naming an older plugin version (the cache path carries the version, so every plugin update stales it and pruning that version makes it dangle). Both apply the same ownership test as the sweep, and both go through **one** `systemMessage` — SessionStart's stdout contract is a single JSON object, so a second `process.stdout.write` would corrupt the first.
